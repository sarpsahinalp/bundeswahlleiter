...
interface EnhancedTableProps {
  onRequestSort: (
    ...
    property: keyof ParliamentMember
  ) => void;
  order: Order;
  orderBy: string;
}
...
export default function ParliamentMembers() {
  ...
  const loadMandate = async (year: number, bundesland_id: number) => {
    try {
      const response = await electionApi.getParliamentMembers(
        year,
        bundesland_id
      );
      response.sort((mA, mB) => mA.partei.localeCompare(mB.partei));
      setMandate(
        response.map((mandat, index) => ({
          party: mandat.partei,
  ...
  useEffect(() => {
    const loadData = async () => {
      await loadBundesLander(); // Wait for completion
      if (land) {
        // Now uses updated state
        await loadMandate(year, land.id);
      }
    };
    loadData();
  }, []);

  // Add this to handle subsequent land/year changes
  useEffect(() => {
    if (land) {
      loadMandate(year, land.id);
    }
  }, [year, land]);
