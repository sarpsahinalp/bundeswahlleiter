...
public class V10__init_erststimme_aggr extends BaseJavaMigration {

    private static final String CSV_FILE_PATH = "src/main/resources/electionData/targetCSV/erstestimme.csv";

    @Override
    public void migrate(Context context) {
        // Define the batch insert SQL with placeholders for the parameters
        String insertSql = "INSERT INTO erststimme_aggr (id, partei_id, wahlkreis_id, jahr, stimmen) " +
                "VALUES (nextval('erststimme_aggr_seq'), (select partei.id from partei where partei.kurzbezeichnung = ?), ?, ?, ?)";
        ...
        try (PreparedStatement preparedStatement = context.getConnection().prepareStatement(insertSql)) {
            ...
            rows.forEach(row -> {
                try {
                    ...
                    preparedStatement.setString(1, row[1].trim()); // partei.name
                    preparedStatement.setLong(2, Long.parseLong(row[0].trim())); // wahlkreis_id
