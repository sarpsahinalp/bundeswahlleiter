...
public class V6__init_kandidaten_2017 extends BaseJavaMigration {

    private static final String CSV_FILE_PATH = "src/main/resources/electionData/sourceCSV/2017/candidate2017.csv";
        ...
        try (PreparedStatement preparedStatement = context.getConnection().prepareStatement(insertSql)) {
            ...
            rows.forEach(row -> {
                try {
                    ...
                    preparedStatement.setString(6, row[5].trim()); // bundesland_name
                    if (row[6].isBlank()) {
                        preparedStatement.setNull(7, java.sql.Types.INTEGER); // landesliste_platz
                    } else {
                        preparedStatement.setLong(8, Long.parseLong(row[6].trim())); // landesliste_platz
                    }
